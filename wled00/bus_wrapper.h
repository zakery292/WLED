...
#elif !defined(CONFIG_IDF_TARGET_ESP32C3) && !defined(CONFIG_IDF_TARGET_ESP32P4)
...
#endif
// Note: ESP32-P4 (RISC-V) does not support I2S LED output - use RMT only
...
class PolyBus {
  ...
  static uint8_t getI(uint8_t busType, const uint8_t* pins, uint8_t num = 0) {
    ...
    if (Bus::is2Pin(busType)) { //SPI LED chips
      ...
      // ESP32-P4 only has 4 RMT TX channels, no I2S LED support
      if (num > 3) return I_NONE;
      // offset stays 0 = RMT only
      #else
